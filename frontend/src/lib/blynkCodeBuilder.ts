...
const BLYNK_COMPONENT_MAP: Record<string, BlynkComponentConfig> = {
  ...
  Sensor_PIR: {
    ...
    sensorReadCode: (vPin, sfx) => [
      ...
      `  if (pir${sfx} && !pirLast${sfx}) Blynk.logEvent("motion_detected", "🚨 Motion detected!");`,
      `  pirLast${sfx} = pir${sfx};`,
  ...
  Sensor_MQ2_Gas: {
    ...
    sensorReadCode: (vPin, sfx) => [
      ...
      `  if (gasHigh${sfx} && !gasAlerted${sfx}) { Blynk.logEvent("gas_alert", "⚠️ High gas level detected!"); gasAlerted${sfx} = true; }`,
  ...
  Sensor_Soil_Moisture: {
    ...
    sensorReadCode: (vPin, sfx) => [
      ...
      `  if (soilPct${sfx} < 30 && !soilAlerted${sfx}) { Blynk.logEvent("dry_soil", "🏜️ Soil is dry — needs water!"); soilAlerted${sfx} = true; }`,
  ...
  Sensor_Rain: {
    ...
    sensorReadCode: (vPin, sfx) => [
      ...
      `  if (rainWet${sfx} && !rainAlerted${sfx}) { Blynk.logEvent("rain_alert", "🌧️ Rain detected!"); rainAlerted${sfx} = true; }`,
  ...
  Sensor_Flame: {
    ...
    sensorReadCode: (vPin, sfx) => [
      ...
      `  if (flameDet${sfx} && !flameAlerted${sfx}) { Blynk.logEvent("flame_alert", "🔥 FLAME DETECTED!"); flameAlerted${sfx} = true; }`,
